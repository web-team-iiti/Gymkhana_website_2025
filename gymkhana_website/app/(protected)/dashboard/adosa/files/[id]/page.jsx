...
import { 
    FaUser, FaCalendarAlt, FaHistory, FaFileAlt, 
    FaExternalLinkAlt, FaFilePdf, FaArrowLeft // Added FaArrowLeft
} from "react-icons/fa";
...
export default async function AdosaReviewPage({ params }) {
    ...
    if (!proposal) return (
        <div className="flex flex-col items-center justify-center min-h-[50vh] text-gray-400 p-6 text-center">
            ...
            <Link href="/dashboard/adosa/files" className="mt-6 bg-gray-800 text-white px-4 py-2 rounded-lg text-sm hover:bg-gray-700">
                Go Back
            </Link>
        </div>
    );
    ...
    return (
        <div className="max-w-7xl mx-auto p-4 md:p-8 pb-20 grid grid-cols-1 gap-6 md:gap-8">
            
            {/* --- 🔙 BACK BUTTON --- */}
            <div className="flex items-center">
                <Link 
                    href="/dashboard/adosa/files" 
                ...
                <div className="lg:col-span-4 space-y-6">
                    ...
                    <div className="bg-gray-900 border border-gray-800 p-6 rounded-2xl shadow-xl">
                        ...
                        <div className="space-y-6 relative pl-2">
                            {logs.length > 0 && <div className="absolute left-[29px] top-4 bottom-4 w-[2px] bg-gray-800 z-0"></div>}
                            ...
                            {logs.map((log, index) => (
                                <div key={log.id} className="relative z-10 flex gap-4">
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        {log.remark && <p className="text-xs text-gray-400 italic mt-1 break-words">"{log.remark}"</p>}
                                    </div>
                                </div>
                            ))} 
                        </div>
