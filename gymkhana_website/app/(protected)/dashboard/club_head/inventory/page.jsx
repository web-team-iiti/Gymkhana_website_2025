...
import { auth } from "@/auth"; // 👈 Import auth to get the user's club
...
import Link from "next/link"; // Required for the "Add Item" button

// Data Fetching Logic (Now filters by specific club)
async function getClubInventory(clubName, searchText) {
...
export default async function ClubInventoryPage({ searchParams }) {
    ...
    // Handle case where user is a "Club Head" but not assigned to a club row in DB
    if (!clubName) {
        return (
            <div className="p-10 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <p className="text-gray-400">
                    Your account has the 'Club Head' role, but is not linked to a specific club.<br/>
                    Please contact the General Secretary to fix your account linkage.
                </p>
            </div>
        );
    }

    // 2. Fetch Data (Locked to their Club)
    const items = await getClubInventory(clubName, searchText);
