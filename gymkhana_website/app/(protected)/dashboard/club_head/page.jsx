  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-4">
        <div className="p-3 bg-blue-500/10 rounded-xl border border-blue-500/50 text-blue-400">
            <FaUserGraduate size={32} />
        </div>
        <div>
            <h1 className="text-3xl font-bold text-white">Club Head Dashboard</h1>
            <p className="text-gray-400">Manage your club events and proposals.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        />
        <StatsCard icon={<FaCheckCircle />} color="text-green-500" border="border-gray-800" title="Approved" value={stats.approved_count} label="completed" />
        <StatsCard icon={<FaTimesCircle />} color="text-red-500" border="border-gray-800" title="Rejected" value={stats.rejected_count} label="archived" />
      </div>
...
const StatsCard = ({ icon, color, bg, border, title, value, label, shadow }) => (
  <div className={`bg-gray-900 border ${border || 'border-gray-800'} p-6 rounded-2xl relative overflow-hidden group hover:border-gray-600 transition-all ${shadow}`}>
...
const RecentList = ({ files, role, theme }) => (
  <div className={`bg-gray-900 border border-gray-800 rounded-2xl p-6`}>
    <div className="flex justify-between items-center mb-6">
      ...
      <Link href={`/dashboard/${role}/my-proposals`} className={`text-xs text-${theme}-400 hover:text-white flex items-center gap-1`}>View All <FaArrowRight /></Link>
    </div>
        ...
        files.map(file => (
          <div key={file.id} className="flex items-center justify-between p-4 bg-gray-950/50 rounded-xl border border-gray-800 hover:border-gray-700 transition-colors">
            <div>
              ...
              <p className={`text-xs text-${theme}-400 mt-1`}>{file.status.replace('_', ' ')}</p>
            </div>
...
const QuickGuide = ({ theme }) => (
  <div className={`bg-gradient-to-br from-gray-900 to-gray-800 border border-gray-800 rounded-2xl p-6 flex flex-col justify-center`}>
    ...
    <ul className="space-y-4 text-sm text-gray-400">
      <li className="flex gap-3"><div className={`bg-gray-800 p-2 rounded text-${theme}-400`}><FaFileAlt /></div><span><strong className="text-gray-200 block">Create Proposal</strong>Submit new events or budget requests.</span></li>
      <li className="flex gap-3"><div className={`bg-gray-800 p-2 rounded text-${theme}-400`}><FaCheckCircle /></div><span><strong className="text-gray-200 block">Check Status</strong>See where your file is stuck in the pipeline.</span></li>
    </ul>
  </div>
);