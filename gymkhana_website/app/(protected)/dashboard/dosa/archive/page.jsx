...
async function getArchivedFiles(searchQuery) {
  let sql = `
  ...
  const params = [];
  if (searchQuery) {
    sql += ` AND (p.title ILIKE $1 OR u.name ILIKE $1)`;
...
export default async function DosaArchivePage({ searchParams }) {
  ...
  const files = await getArchivedFiles(q);
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8 pb-20">
      ...
      <div className="flex flex-col lg:flex-row justify-between lg:items-end gap-6">
        ...
        <div className="grid grid-cols-2 gap-3 w-full lg:w-auto">
            <div className="bg-green-500/10 border border-green-500/20 px-4 py-3 rounded-xl flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3">
            ...
            <div className="bg-red-500/10 border border-red-500/20 px-4 py-3 rounded-xl flex flex-col md:flex-row items-start md:items-center gap-2 md:gap-3">
