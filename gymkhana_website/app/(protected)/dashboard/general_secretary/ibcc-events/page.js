...
export default async function GSSNTIBCCEventsPage() {
  ...
  // Fetch clubs to assign events (Technical & Cultural Clubs)
  const clubsRes = await query("SELECT club_id as id, club_name as name FROM clubs WHERE category IN ('Technical Clubs', 'Technical Teams', 'Cultural Clubs')");
