...
import { query } from "@/config/db"; // <--- THIS WAS MISSING
import { FaBuilding, FaClock, FaExclamationCircle, FaCheckCircle, FaArrowRight, FaFileAlt } from "react-icons/fa";
...
async function getStats() {
  ...
  const urgentFilesQuery = `
    ...
    WHERE current_stage = 'OFFICE_REVIEW' AND priority IN ('URGENT', 'HIGH')
    ORDER BY proposals.created_at ASC LIMIT 3;
  `;
    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-gray-400 mt-1">Overview of incoming gymkhana files.</p>
        </div>
      </div>
      ...
      {/* Urgent List & Quick Guide */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <UrgentList files={urgentFiles} role="office" />
        <QuickGuide />
      </div>
    </div>
  );
