...
//                   {p.tenure_start && p.tenure_end && (
//                     <p className="text-xs text-gray-500 mt-1">
//                       {new Date(p.tenure_start).toDateString()} →{" "}
//                       {new Date(p.tenure_end).toDateString()}
//                     </p>
//                   )}
...
export default async function StudentDashboard() {
  ...
  const hasVerifiedPOR = pors.some(p => p.status === "APPROVED");
            ...
            {pors.map(p => (
              ...
              >
                <div>
                  ...
                  {p.status === "TENURE_ADDED" || p.status === "APPROVED" && (
                    <span className="text-green-400 flex items-center gap-2 text-xs mt-2">
                      <FaCheckCircle /> Verified
                    </span>
                  )}
