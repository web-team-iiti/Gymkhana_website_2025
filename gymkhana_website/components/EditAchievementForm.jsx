...
import { FaCloudUploadAlt, FaCalendarAlt, FaArrowLeft, FaTimes, FaSave } from "react-icons/fa";

export default function EditAchievementForm({ councils, achievement }) {
  const router = useRouter();
  const [loading, setLoading] = useState(false);
  
  // Convert date format from ISO to YYYY-MM-DD for the date input
  const initialDate = new Date(achievement.achievement_date).toISOString().split('T')[0];

  const [formData, setFormData] = useState({
    title: achievement.title || "",
    council_id: achievement.council_id || (councils.length > 0 ? councils[0].id : ""),
    date: initialDate,
    description: achievement.description || "",
  });
  
  const [image, setImage] = useState(null);
  const [preview, setPreview] = useState(achievement.image_url || null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        const res = await fetch(`/api/achievements/${achievement.id}/edit`, { method: "PUT", body: data });
        if (res.ok) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 md:p-8 pb-20">
      <div className="mb-6">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-white">Edit Achievement</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 md:p-8 shadow-xl space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="title" value={formData.title} required className="w-full bg-gray-950 border border-gray-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-yellow-500" onChange={handleChange} />
            </div>
            <div>
        ...
        <div>
            ...
            <div className="relative">
                ...
                <input type="date" name="date" value={formData.date} required className="w-full bg-gray-950 border border-gray-800 text-white rounded-xl pl-10 pr-4 py-3 focus:outline-none focus:border-yellow-500 appearance-none" onChange={handleChange} />
            </div>
        </div>

        <div>
            <label className="block text-xs font-bold text-gray-400 uppercase tracking-wide mb-2">Description</label>
            <textarea name="description" rows="4" value={formData.description} required className="w-full bg-gray-950 border border-gray-800 text-white rounded-xl px-4 py-3 focus:outline-none focus:border-yellow-500 resize-none" onChange={handleChange} />
        </div>
            ...
            <div className="grid grid-cols-2 gap-4 max-w-sm">
                {preview ? (
                    ...
                    <label className="aspect-video border-2 border-dashed border-gray-700 bg-gray-950/50 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-yellow-500 hover:text-yellow-500 text-gray-500">
                        ...
                        <span className="text-xs font-bold">New Image</span>
